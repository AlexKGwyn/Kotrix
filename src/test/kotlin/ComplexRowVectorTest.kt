...
internal class ComplexRowVectorTest {
    private val vec = ComplexRowVector(4, arrayOf(1.I, 2.R, 3.I, 4.R))
    private val minusVec = ComplexRowVector(4, arrayOf((-1).I, (-2).R, (-3).I, (-4).R))
    private val twiceVec = ComplexRowVector(4, arrayOf(2.I, 4.R, 6.I, 8.R))
    private val mat = ComplexMatrix(4, 4) { i, j -> if (i == j) i + 1.R else 0.R}
    ...
    @Test
    fun times() {
        val vec1 = ComplexRowVector(4, arrayOf(1.I, 4.R, 9.I, 16.R))
        assertEquals(twiceVec, 2 * vec)
        assertEquals(twiceVec, vec * 2)
        assertEquals(vec1, vec * mat)
    }
    ...
    @Test
    fun getSubvector() {
        val subVec = ComplexRowVector(2, arrayOf(2.R, 3.I))
        assertEquals(subVec, vec.getSubvector(1, 3))
    }

    @Test
    fun setSubvector() {
        val subVec = ComplexRowVector(2, arrayOf(2.R, 3.R))
        val vec1 = ComplexRowVector(4, arrayOf(1.I, 2.R, 3.I, 4.R))
        val vec2 = ComplexRowVector(4, arrayOf(1.I, 2.R, 3.R, 4.R))
    ...
    @Test
    fun eltwiseMul() {
        val vec1 = ComplexRowVector(4, arrayOf((-1).R, 4.R, (-9).R, 16.R))
    ...
    @Test
    fun crossProduct() {
        val vec1 = ComplexRowVector(3, arrayOf(1.R, 2.R, 3.R))
        assertEquals(ComplexRowVector(3), vec1.crossProduct(vec1))
    }
